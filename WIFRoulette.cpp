...
const char WIF_ENDING[] = "5bCRZhiS5sEGMpmcRZdpAhmWLRfMmutGmPHtjVob";
...
void generate_valid_prefix(char* output, Xoshiro256plus& rng) {
    ...
    // Copy the selected prefix into the output buffer
    strcpy(output, VALID_PREFIXES[random_index]);
}
...
void init_batch(WIFBatch& batch, int batch_size) {
    ...
    char random_prefix[PREFIX_LENGTH + 1];
    char random_root[ROOT_LENGTH + 1];

    for (int b = 0; b < batch_size; ++b) {

        generate_valid_prefix(random_prefix, rng);

        generate_random_root(random_root, ROOT_LENGTH, rng);

        memcpy(batch.wifs[b], random_prefix, PREFIX_LENGTH);
        memcpy(batch.wifs[b] + PREFIX_LENGTH, random_root, ROOT_LENGTH);
        memcpy(batch.wifs[b] + PREFIX_LENGTH + ROOT_LENGTH, WIF_ENDING, strlen(WIF_ENDING) + 1);
